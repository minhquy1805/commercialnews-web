...
export function getPageInfo(
  ...
  const page = normalizePositiveNumber(sourcePageInfo?.page, fallback.page);
  const pageSize = normalizePositiveNumber(
    sourcePageInfo?.pageSize,
    fallback.pageSize,
  );
  const totalItems = normalizePositiveNumber(sourcePageInfo?.totalItems, 0);
  ...
  return {
    ...
    totalItems,
    totalPages:
      sourcePageInfo?.totalPages ?? calculateTotalPages(totalItems, pageSize),
  };
}
